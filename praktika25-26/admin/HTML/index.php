<?php 
    include '../dev-tools/database.php';
    $ip = 'localhost';
    $username = 'root';
    $password = '';
    $dbName = 'asentus';
    ...
    if(isset($_POST['service'])) {
        $result = DB_query("UPDATE `services` SET `header`='{$_POST['header']}',`text`='{$_POST['text']}' WHERE `id` = {$_POST['id']}", $dataBase);
        header('Location: index.php');
    }

    if(isset($_POST['latestProducts'])) {
        if (!empty($_FILES['image']['name'])) {
            $file = "img/". $_FILES['image']['name'];
            move_uploaded_file($_FILES['image']['tmp_name'], $file);
            $result = DB_query("UPDATE `latestProducts` SET `image`='$file',`h4_a`='{$_POST['h4_a']}',`h4_sp`='{$_POST['h4_sp']}',`text`='{$_POST['text']}' WHERE `id` = {$_POST['id']}", $dataBase);
        }
        else $result = DB_query("UPDATE `latestProducts` SET `h4_a`='{$_POST['h4_a']}',`h4_sp`='{$_POST['h4_sp']}',`text`='{$_POST['text']}' WHERE `id` = {$_POST['id']}", $dataBase);
        header('Location: index.php');
    }

    if(isset($_POST['clients'])) {
        if (!empty($_FILES['image']['name'])) {
            $file = "img/clients". $_FILES['image']['name'];
            move_uploaded_file($_FILES['image']['tmp_name'], $file);
            $result = DB_query("UPDATE `clients` SET `image`='$file' WHERE `id` = {$_POST['id']}", $dataBase);
        }
        else $result = DB_query("UPDATE `clients` SET `image`='$file' WHERE `id` = {$_POST['id']}", $dataBase);
        header('Location: index.php');
    }
...
<html lang="en" class="no-js">
    ...
    <body>
        ...
        <div class="bg-color-sky-light" data-auto-height="true">
            <div class="content-lg container">
                <div class="row row-space-1 margin-b-2">
                    <?php 
                        $result = DB_query('SELECT * FROM `services` WHERE `row` = 1;', $dataBase);
                        $elementsRow1 = DB_getMas($result);
                        foreach($elementsRow1 as $key => $value) { ?>
                            <div class="col-sm-4 sm-margin-b-2">
                                <div class="wow fadeInLeft" data-wow-duration=".3" data-wow-delay=".3s">
                                    <div class="service" data-height="height">
                                        <div class="service-element">
                                            <i class="service-icon <?= $value['icon']; ?>"></i>
                                        </div>
                                        <div class="service-info">
                                            <h3><?= $value['header']; ?></h3>
                                            <p class="margin-b-5"><?= $value['text']; ?></p>
                                        </div>
                                        <form action="" method="post">
                                            <input type="hidden" name="id" value="<?= $value['id']; ?>">
                                            <input type="text" placeholder='Header' name="header" value="<?= $value['header']; ?>"><br>
                                            <input type="text" placeholder='Text' name="text" value="<?= $value['text']; ?>"><br>
                                            <input type="submit" name="service" value="Изменить">
                                        </form> 
                                    </div>
                                </div>
                            </div>
                ...
                <div class="row row-space-1">
                    <?php 
                        $result = DB_query('SELECT * FROM `services` WHERE `row` = 2;', $dataBase);
                        $elementsRow2 = DB_getMas($result);
                        foreach($elementsRow2 as $key => $value) { ?>
                            <div class="col-sm-4 sm-margin-b-2">
                                <div class="wow fadeInLeft" data-wow-duration=".3" data-wow-delay=".3s">
                                    <div class="service" data-height="height">
                                        <div class="service-element">
        ...
        <div class="content-lg container">
            ...
            <div class="row">
                <?php 
                    $result = DB_query('SELECT * FROM `latestProducts`', $dataBase);
                    $latestProducts = DB_getMas($result);
                    foreach($latestProducts as $key => $value) { ?>
                        <div class="col-sm-4 sm-margin-b-50">
                            <div class="margin-b-20">
                                <div class="wow zoomIn" data-wow-duration=".3" data-wow-delay=".1s">
                                    <img class="img-responsive" src="<?= $value['image']; ?>" alt="Latest Products">
                                </div>
                            </div>
                            <h4><a href="#"><?= $value['h4_a']; ?></a> <span class="text-uppercase margin-l-20"><?= $value['h4_sp']; ?></span></h4>
                            <p><?= $value['text']; ?></p>
                            <a class="link" href="#">Read More</a>
                            <form action="" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="id" value="<?= $value['id']; ?>">
                                <input type="file" name="image" >
                                <input type="text" placeholder='Header' name="h4_a" value="<?= $value['h4_a']; ?>"><br>
                                <input type="text" placeholder='Header category' name="h4_sp" value="<?= $value['h4_sp']; ?>"><br>
                                <input type="text" placeholder='Text' name="text" value="<?= $value['text']; ?>"><br>
                                <input type="submit" name="latestProducts" value="Изменить">
                            </form>
                        </div>
                <?php } ?>
            </div>
            ...
            <div class="content-lg container">
                ...
                <div class="swiper-slider swiper-clients">
                    ...
                    <div class="swiper-wrapper">
                        <?php 
                            $result = DB_query('SELECT * FROM `clients`', $dataBase);
                            $clients = DB_getMas($result);
                            foreach($clients as $key => $value) { ?>
                                <div class="swiper-slide">
                                    <img class="swiper-clients-img" src="<?= $value['image']; ?>" alt="Clients Logo">
                                    <form action="" method="post" enctype="multipart/form-data">
                                        <input type="hidden" name="id" value="<?= $value['id']; ?>">
                                        <input type="file" name="image" >
                                        <input type="submit" name="clients" value="Изменить">
                                    </form>
                                </div>
                        <?php } ?>
                    </div>
